...
struct Missionary_FlashApp: App {
  ...
  private func importJSONData() {
      ...
      // Import JSON data
    guard let url = Bundle.main.url(forResource: "missionaries", withExtension: "json") else {
    ...
    do {
        ...
        let missionaries = try JSONDecoder().decode([Missionary].self, from: jsonData)
    ...
    private func parseJSON(_ jsonData: Data) throws -> [Missionary] {
        let decoder = JSONDecoder()

        do {
            // Decode the JSON into an array of dictionaries
            let missionaries = try decoder.decode([Missionary].self, from: jsonData)
            return missionaries
        } catch {
